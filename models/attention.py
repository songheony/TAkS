...
class Attention(nn.Module):
    ...
    def __init__(self, block_num, grayscale=False, class_num=100):

        super().__init__()

        image_channel = 1 if grayscale else 3
        self.pre_conv = nn.Sequential(
            nn.Conv2d(image_channel, 64, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
        )
...
def attention56(grayscale=False, num_classes=100):
    return Attention([1, 1, 1], grayscale=grayscale, class_num=num_classes)


def attention92(grayscale=False, num_classes=100):
    return Attention([1, 2, 3], grayscale=grayscale, class_num=num_classes)
