...
class GoogleNet(nn.Module):
    def __init__(self, grayscale=False, num_class=100):
        super().__init__()

        image_channel = 1 if grayscale else 3
        self.prelayer = nn.Sequential(
            nn.Conv2d(image_channel, 192, kernel_size=3, padding=1),
            nn.BatchNorm2d(192),
            nn.ReLU(inplace=True),
        )
...
def googlenet(grayscale=False, num_classes=100):
    return GoogleNet(grayscale=grayscale, num_classes=num_classes)
