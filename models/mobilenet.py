...
class MobileNet(nn.Module):
    ...
    def __init__(self, width_multiplier=1, grayscale=False, class_num=100):
        super().__init__()

        alpha = width_multiplier
        image_channel = 1 if grayscale else 3
        self.stem = nn.Sequential(
            BasicConv2d(image_channel, int(32 * alpha), 3, padding=1, bias=False),
...
def mobilenet(alpha=1, grayscale=False, num_classes=100):
    return MobileNet(alpha, grayscale=grayscale, class_num=num_classes)
